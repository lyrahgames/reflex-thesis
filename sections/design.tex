...
\label{sub:polyhedral_surface_data_structure}
  ...
  In the context of computer graphics, vertices of a polyhedral surface must differ as soon as they exhibit at least one distinct attribute.
  In the structure given above, this would mean even though triangles might reference the same vertex position their pseudo-normals might differ to model a crease in the surface and consequently they cannot reference the same vertex.
  Even worse, it is allowed to use multiple instances of the same vertex inside the vertex list to improve cache efficiency.
  In the extreme case of STL files, every triangle refers to vertices that cannot be referred by other triangles.
  As a consequence, naively generating the graph or dual graph of the surface mesh solely based on the vertices and faces used for rendering will not result in the correct topological connectivity information.

  Referring to surface mesh curves, the values for pseudo-normals or texture coordinates are unimportant and may be neglected as they do not change the topology of the underlying polyhedral surface.
  Therefore curve algorithms would need to assume two vertices to be the same as long as they exhibit the same position.
  In this sense, I call two vertices with this property to be topologically equivalent.
  Two triangles are topologically adjacent to each other if for both there exists edges.



  Unfortunately, the connectivity information provided by the faces and vertices are not suitable for surface mesh curves but only useful in the context of rendering.
  ...
  As as consequence, a topological structure has to be generated to extend the connectivity given by the render structure.
  ...
  The projection of vertices onto their positions can be modeled by the quotient space and its respective quotient map.
  $\function{p}{\mathscr{V}}{\setReal^3}$
  $v,w\in\mathscr{V}$
  \[
    v \sim w
    \colon\iff
    p(v) = p(w)
  \]
  $\mathscr{V}/\sim$ is the quotient space which contains all topological vertices.
  Its elements are classes that contain topologically equivalent vertices.
  \[
