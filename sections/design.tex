...
\label{sub:polyhedral_surface_data_structure}
  ...
  \[
    ...
    τ(v) \define [v]
  \]
  Two triangles are topologically adjacent to each other if they share a topological edge.
  Using the quotient map, a rigorous formulation can be given as follows.
  Let $n\in\setNatural$ be the number of vertices in the vertex list $\function{V}{\mathscr{I}}{\mathscr{V}}$ with $\mathscr{I}\define\set{k\in\setNatural_0}{k\leq n}$.
  Two triangles given by index triples $(v_1,v_2,v_3)$ and $(w_1,w_2,w_3)$ are adjacent to each other if there are $i,j,p,q\in\set{1,2,3}{}$ with $i\neq j$ and $p\neq q$ such that
  \[
    (τ(V(v_i)),τ(V(v_j))) = (τ(V(w_p)),τ(V(w_q)))
  \]
