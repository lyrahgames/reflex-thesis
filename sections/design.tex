...
\label{sub:discrete_surface_curve_data_structure}
  ...
  As the surface mesh is the union of topological edges, a natural construction would be based on a list of edges which contain the control points of the curve, respectively.
  ...
  Presumably, the code snippet above describes one of the easiest possible data structures for surface mesh curves.
  But the simple edge structure does not offer the ability to simply access adjacent edges or faces without the use of the temporary hash map of directed edges.
  To cope with this problem, one could either use a quad-edge algebra or base all operations done with the curve on the vertex neighbor structure.
  The edge uses references to topological vertices and will always point to the right side of the curve.
  Closed curves can be characterized by making the last point the same as the first.
  One bigger problem is the regularity condition check and regularization which does not seem to be possible without the undirected edge hash map.
  To check for regularity, the next edge is not allowed to be the current, previous, or the difference edge.
  If the control point needs to represent a single vertex of the surface, both edge references will point to the same topological vertex.
