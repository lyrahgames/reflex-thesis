...
\label{sub:differential_geometry}
  ...
  \begin{definition}[Straightest Geodesic]
  ...
  \begin{definition}[Locally Shortest Geodesic]
    Let $M$ be a Riemannian submanifold embedded in Euclidean space and γ be a curve embedded in $M$ and parameterized by arc length.
...
\label{sub:polyhedral_surfaces}
  ...
  \begin{figure}
    ...
    \caption[Examples of Polyhedral Surfaces]{%
      ...
      The models for the Hilbert Cube, the Julia Vase, and the Buddha have been taken from the \citetitle{thingi10k} dataset \autocite{thingi10k}.
    }
  ...
  \begin{definition}[Total Vertex Angle and Total Gauss Curvature]
    Let $\triangle$ be a triangle and $A,B,C$ its vertices.
    % The total vertex angle $ϑ_\triangle$ of $\triangle$ is defined as follows.
  ...
  % Here, it can the previous can be seen for locally shortest geodesics.
  % representable a by a sequence of $n$ triangles, $n-1$ real values, and start/end points.
  ...
  % \begin{lemma}[Locally shortest geodesics are regular surface mesh curves]
  %   Let
  % \end{lemma}
  ...
  \begin{definition}[Oriented Curve Angle]
  ...
  \begin{definition}[Discrete Straightest Geodesic]
    Let $(S,ω)$ be an oriented polyhedral surface and γ be a regular surface mesh curve on $S$.
  ...
  \begin{proposition}[Properties of Discrete Geodesics]
    \vspace{-2em}
    \begin{enumerate}
